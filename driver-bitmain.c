...
static bool get_options(int this_option_offset, int *baud, int *chain_num, int *asic_num, int *timeout, int *frequency, char *frequency_t, uint8_t * reg_data, uint8_t * voltage, char *voltage_t)
{
	char buf[BUFSIZ + 1];
	...
	case 19200:
		*baud = 19200;
		break;
	default:
		quit(1, "Invalid bitmain-options for baud (%s) " "must be 115200, 57600, 38400 or 19200", buf);
	}
		...
		if (*colon) {
			...
			if (tmp > 0) {
				*chain_num = tmp;
				...
				quit(1, "Invalid bitmain-options for " "chain_num (%s) must be 1 ~ %d", colon, BITMAIN_DEFAULT_CHAIN_NUM);
			}
		}
		...
		if (colon2 && *colon2) {
			...
			if (tmp > 0 && tmp <= BITMAIN_DEFAULT_ASIC_NUM)
				*asic_num = tmp;
				...
				quit(1, "Invalid bitmain-options for " "asic_num (%s) must be 1 ~ %d", colon2, BITMAIN_DEFAULT_ASIC_NUM);
			}
			...
			if (colon3 && *colon3) {
				...
				if (tmp > 0 && tmp <= 0xff)
					*timeout = tmp;
					...
					quit(1, "Invalid bitmain-options for " "timeout (%s) must be 1 ~ %d", colon3, 0xff);
				}
				if (colon4 && *colon4) {
					...
					if (tmp < BITMAIN_MIN_FREQUENCY || tmp > BITMAIN_MAX_FREQUENCY) {
						quit(1, "Invalid bitmain-options for frequency, must be %d <= frequency <= %d", BITMAIN_MIN_FREQUENCY, BITMAIN_MAX_FREQUENCY);
					} else {
						*frequency = tmp;
					...
					if (colon5 && *colon5) {
						...
						if (strlen(colon5) > 8 || strlen(colon5) % 2 != 0 || strlen(colon5) / 2 == 0) {
							quit(1, "Invalid bitmain-options for reg data, must be hex now: %s", colon5);
						}
						memset(reg_data, 0, 4);
						if (!hex2bin(reg_data, colon5, strlen(colon5) / 2)) {
							quit(1, "Invalid bitmain-options for reg data, hex2bin error now: %s", colon5);
						}

						if (colon6 && *colon6) {
							if (strlen(colon6) > 4 || strlen(colon6) % 2 != 0 || strlen(colon6) / 2 == 0) {
								quit(1, "Invalid bitmain-options for voltage data, must be hex now: %s", colon6);
							}
							memset(voltage, 0, 2);
							if (!hex2bin(voltage, colon6, strlen(colon6) / 2)) {
								quit(1, "Invalid bitmain-options for voltage data, hex2bin error now: %s", colon5);
							} else {
...
static bool get_option_freq(int *timeout, int *frequency, char *frequency_t, uint8_t * reg_data)
{
	...
	else
	{
		...
		if (tmp < BITMAIN_MIN_FREQUENCY || tmp > BITMAIN_MAX_FREQUENCY) {
			quit(1, "Invalid bitmain-freq for frequency, must be %d <= frequency <= %d", BITMAIN_MIN_FREQUENCY, BITMAIN_MAX_FREQUENCY);
		} else {
			*frequency = tmp;
...
static bool get_option_voltage(uint8_t * voltage, char *voltage_t)
{
	if (opt_bitmain_voltage) {
		if (strlen(opt_bitmain_voltage) > 4 || strlen(opt_bitmain_voltage) % 2 != 0 || strlen(opt_bitmain_voltage) / 2 == 0) {
			applog(LOG_ERR, "Invalid bitmain-voltage for voltage data, must be hex now: %s,set default_volttage", opt_bitmain_voltage);
			return false;
		}
		memset(voltage, 0, 2);
		if (!hex2bin(voltage, opt_bitmain_voltage, strlen(opt_bitmain_voltage) / 2)) {
			quit(1, "Invalid bitmain-voltage for voltage data, hex2bin error now: %s", opt_bitmain_voltage);
		} else {
			sprintf(voltage_t, "%02x%02x", voltage[0], voltage[1]);
			voltage_t[5] = 0;
			voltage_t[4] = voltage_t[3];
			    ...
			    uint8_t chip_address, uint8_t reg_address, uint8_t * reg_data)
{
	...
	if (unlikely(timeout_data <= 0 || asic_num <= 0 || chain_num <= 0)) {
		applog(LOG_WARNING, "bitmain_set_txconfig parameter invalid timeout_data(%d) asic_num(%d) chain_num(%d)", timeout_data, asic_num, chain_num);
		return -1;
	}
...
static int bitmain_set_txtask(uint8_t * sendbuf, unsigned int *last_work_block, struct work **works, int work_array_size, int work_array, int sendworkcount, int *sendcount)
{
	uint16_t crc = 0;
	uint32_t work_id = 0;
	uint8_t version = 0;
	...
	*sendcount = 0;
	...
	applog(LOG_DEBUG, "BitMain TxTask Token: %d %d %02x%02x%02x%02x%02x%02x", datalen, bm->length, sendbuf[0], sendbuf[1], sendbuf[2], sendbuf[3], sendbuf[4], sendbuf[5]);
	...
	*sendcount = cursendcount;
	...
	applog(LOG_DEBUG, "BitMain TxTask Token: v(%d) new_block(%d) diff(%d pool:%d net:%d) work_num(%d) crc(%04x)", version, new_block, diff, pooldiff, netdiff, cursendcount, crc);
	applog(LOG_DEBUG, "BitMain TxTask Token: %d %d %02x%02x%02x%02x%02x%02x", datalen, bm->length, sendbuf[0], sendbuf[1], sendbuf[2], sendbuf[3], sendbuf[4], sendbuf[5]);

	return datalen;
}

static int bitmain_set_rxstatus(struct bitmain_rxstatus_token *bm, uint8_t chip_status_eft, uint8_t detect_get, uint8_t chip_address, uint8_t reg_address)
{
	uint16_t crc = 0;
	uint8_t version = 0;
	int datalen = 0;
	...
	applog(LOG_ERR, "BitMain RxStatus Token: v(%d) chip_status_eft(%d) detect_get(%d) chip_address(%02x) reg_address(%02x) crc(%04x)", version, chip_status_eft, detect_get, chip_address, reg_address, crc);
...
static int bitmain_parse_rxstatus(const uint8_t * data, int datalen, struct bitmain_rxstatus_data *bm)
{
	...
	if (dataindex + bm->temp_num + bm->fan_num + 2 != datalen) {
		applog(LOG_ERR, "bitmain_parse_rxstatus dataindex(%d) chain_num(%d) temp_num(%d) fan_num(%d) not match datalen(%d)", dataindex, bm->chain_num, bm->temp_num, bm->fan_num, datalen);
		return -1;
	}
	for (i = 0; i < bm->chain_num; i++) {
...
static int bitmain_parse_rxnonce(const uint8_t * data, int datalen, struct bitmain_rxnonce_data *bm, int *nonce_num)
{
	...
	for (i = 0; i < curnoncenum; i++) {
		...
		applog(LOG_DEBUG, "BitMain RxNonce Data %d: work_id(%d) nonce(%08x)(%d)", i, bm->nonces[i].work_id, bm->nonces[i].nonce, bm->nonces[i].nonce);
	}
...
static bool bitmain_decode_nonce(struct thr_info *thr, struct cgpu_info *bitmain, struct bitmain_info *info, uint32_t nonce, struct work *work)
{
...
{
	applog(LOG_INFO, "%s%d: No matching work - HW error", thr->cgpu->drv->name, thr->cgpu->device_id);
...
static void bitmain_update_temps(struct cgpu_info *bitmain, struct bitmain_info *info, struct bitmain_rxstatus_data *bm)
{
	char tmp[64] = { 0 };
	char msg[10240] = { 0 };
	int i = 0;
	...
	applog(LOG_DEBUG, "BitMain: temp_index: %d, temp_count: %d, temp_old: %d", info->temp_history_index, info->temp_history_count, info->temp_old);
	...
	for (i = 0; i <= spare; i++) {
		if (buf[i] == 0xa1) {
			...
			if (bitmain_parse_rxstatus(buf + i, packethead.length + 4, &rxstatusdata) != 0) {
				...
				applog(LOG_ERR, "bitmain_parse_results v=%d chain=%d fifo=%d hwv1=%d hwv2=%d hwv3=%d hwv4=%d nerr=%d-%d freq=%d chain info:", rxstatusdata.version, info->chain_num, info->fifo_space, info->hw_version[0], info->hw_version[1], info->hw_version[2], info->hw_version[3], info->last_nonce_error, info->nonce_error, info->frequency);
...
static bool bitmain_prepare(struct thr_info *thr)
{
	...
	free(bitmain->works);
	bitmain->works = calloc(BITMAIN_MAX_WORK_NUM * sizeof(struct work *), BITMAIN_ARRAY_SIZE);
	if (!bitmain->works)
...
static int bitmain_initialize(struct cgpu_info *bitmain)
{
	...
	/* clear read buf */
	ret = bitmain_read(bitmain, data, BITMAIN_READBUF_SIZE, BITMAIN_RESET_TIMEOUT, C_BITMAIN_READ);
	if (ret > 0) {
		if (opt_debug) {
			...
			if (readlen > BITMAIN_READ_SIZE) {
				for (i = 0; i < readlen; i++) {
					if (data[i] == 0xa1) {
						...
						sprintf(g_miner_version, "%d.%d.%d.%d", info->hw_version[0], info->hw_version[1], info->hw_version[2], info->hw_version[3]);
						applog(LOG_ERR, "bitmain_initialize rxstatus v(%d) chain(%d) fifo(%d) hwv1(%d) hwv2(%d) hwv3(%d) hwv4(%d) nerr(%d) freq=%d", rxstatusdata.version, info->chain_num, info->fifo_space, info->hw_version[0], info->hw_version[1], info->hw_version[2], info->hw_version[3], rxstatusdata.nonce_error, info->frequency);
...
static bool bitmain_detect_one(const char *devpath)
{
	...
	applog(LOG_ERR, "BitMain Detected: %s " "(chain_num=%d asic_num=%d timeout=%d freq=%d-%s volt=%02x%02x-%s)", bitmain->device_path, info->chain_num, info->asic_num, info->timeout, info->frequency, info->frequency_t, info->voltage[0], info->voltage[1], info->voltage_t);
...
static bool bitmain_fill(struct cgpu_info *bitmain)
{
 ...
 out_unlock:
	...
	if (timediff > BITMAIN_SEND_STATUS_TIME) {
		...
		sendlen = bitmain_set_rxstatus((struct bitmain_rxstatus_token *)
					       sendbuf, 0, 0, 0, 0);
		if (sendlen > 0) {
			sendret = bitmain_send_data(sendbuf, sendlen, bitmain);
			if (unlikely(sendret == BTM_SEND_ERROR)) {
...
static int64_t bitmain_scanhash(struct thr_info *thr)
{
	...
	if (unlikely(bitmain->usbinfo.nodev)) {
		applog(LOG_ERR, "BTM%d: Device disappeared, shutting down thread", bitmain->device_id);
		bitmain->shutdown = true;
	}
...
char *set_bitmain_freq(char *arg)
{
	...
	if (val1 < BITMAIN_MIN_FREQUENCY || val1 > BITMAIN_MAX_FREQUENCY || val2 < BITMAIN_MIN_FREQUENCY || val2 > BITMAIN_MAX_FREQUENCY || val2 < val1)
